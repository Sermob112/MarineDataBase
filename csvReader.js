...
//                     former_name: row['Бывшее название'],
//                     callsign: row['Позывной'],
//                     port_of_registry: row['Порт приписки'],
//                     flag: row['Флаг'],
//                     class_symbol: row['Символ класса'],
//                     refit: row['Переоборудование/модернизация существенного характера'],
...
//                     significant_part_date: row['Дата значительной части'],
//                     significant_part: row['Значительная часть'],
...
//                     theoretical_length: row['Длина наибольшая (теоретическая)'],
//                     overall_length: row['Длина габаритная'],
//                     design_length: row['Длина расчетная'],
//                     width: row['Ширина габаритная'],
//                     height: row['Высота борта'],
//                     draft: row['Осадка'],
//                     speed: row['Скорость'],
//                     propulsion_type: row['Тип силовой установки'],
//                     main_engines: row['Главные двигатели'],
...
//                     radio_navigation_equipment: row['Радио-навигационное оборудование'],
//                     refrigerated_cargo_spaces: row['Охлаждаемые грузовые помещения'],
...
//                     organization_group: row['Орг. группа'],
...
async function importVesselData() {
    ...
    fs.createReadStream('mergedRegisters (2).csv')
        .pipe(csv({ separator: ';' }))
        .on('data', (row) => {
            const cleanRow = Object.keys(row).reduce((acc, key) => {
        ...
        .on('end', async () => {
            try {
                await MarinFleet.bulkCreate(results); // Добавляем все записи за один раз
                console.log('All data successfully added to the database.');
            } catch (error) {
